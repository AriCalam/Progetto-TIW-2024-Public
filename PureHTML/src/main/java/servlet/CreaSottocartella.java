	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		HttpSession session = request.getSession();
  		...
  		try {
  			...
  			if(nomeCartella.length()>16 || nomeCartella.length()<1) {
  				String path1;
  	  			request.setAttribute("LunghezzaS", "Non hai rispettato la lunghezza minima/massima dei campi, riprova");
  		...
  		if (idPadre == null || idPadre.isEmpty()) {
  			response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Id cartella padre mancante");
  			return;
  		}
		...
		if (c == null) {
			...
			try {
				...
				//Reindirizzamento
				request.setAttribute("SottocartellaCreata", "Cartella creata con successo");
			...
			//Reindirizzamento
			request.setAttribute("SottocartellaEsistente", "Cartella già esistente");
			path = "/GoToGestioneContenuti";
			request.getRequestDispatcher(path).forward(request, response);
		}
	}
