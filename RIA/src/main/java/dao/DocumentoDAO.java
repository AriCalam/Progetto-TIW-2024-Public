...
public class DocumentoDAO {
	...
	//Cerca nel db un documento tramite il suo nome
	public Documento findDocByNome(String nomeDoc, String usr) throws SQLException {
		Documento d = null;
		String query = "SELECT * FROM new_documento WHERE nomeDocumento = ? AND proprietario = ?";
		ResultSet result = null;
		PreparedStatement pstatement = null;
		try {
			pstatement = con.prepareStatement(query);
			pstatement.setString(1, nomeDoc);
			pstatement.setString(2, usr);
			result = pstatement.executeQuery();
			while (result.next()) {
				d = new Documento();
	...
	public Documento checkNome2(String usr, String nome) throws SQLException {
	...
	//Metodo che elimina un documento
	public void eliminaDocById(int id, String username) throws SQLException {
		String query = "DELETE FROM new_documento WHERE idDocumento = ? AND proprietario = ?";
		PreparedStatement pstatement = null;
		try {
			pstatement = con.prepareStatement(query);
